...
import uk.gov.hmrc.excisemovementcontrolsystemapi.repository.{ExciseNumberQueueWorkItemRepository, MovementMessageRepository}
...
class SchedulePollingMessagesWithWorkItemItSpec extends PlaySpec
  with DefaultPlayMongoRepositorySupport[WorkItem[ExciseNumber]]
  ...
  lazy val repo = new MovementMessageRepository(
    mongoComponent,
    appConfig,
    timeService
  )
  ...
  override def beforeEach(): Unit = {
    ...
    when(timeService.now).thenReturn(availableBefore)
  }
  ...
  "Scheduler" should {
    "start Polling show new message copy" in {
      ...
      repo.save(Movement("token", "1", None, None, Seq.empty)).futureValue
      repo.save(Movement("token", "3", None, None, Seq.empty)).futureValue
      repo.save(Movement("token", "4", None, None, Seq.empty)).futureValue
      ...
      val result: Seq[WorkItem[ExciseNumber]] = findAll().futureValue

      val movements = repo.collection.find.toFuture().futureValue

      movements.size mustBe 3
      assertResults(movements.find(_.consignorId.equals("1")).get, Movement("token", "1", None, Some("tokentokentokentokent"), expectedMessage))
      assertResults(movements.find(_.consignorId.equals("3")).get, Movement("token", "3", None, Some("tokentokentokentokent"), expectedMessage.take(1)))
      assertResults(movements.find(_.consignorId.equals("4")).get, Movement("token", "4", None, Some("tokentokentokentokent"), Seq(createMessage(Ie704XmlMessage.IE704, MessageTypes.IE704.value))))
    }
  }
  ...
  private def createWorkItem(ern: String) = {
    WorkItem(
      id           = new ObjectId(),
