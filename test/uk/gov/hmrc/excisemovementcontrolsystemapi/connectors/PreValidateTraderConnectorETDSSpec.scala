...
class PreValidateTraderConnectorETDSSpec
    extends PlaySpec
  ...
  "post" should {
    ...
    "get URL from appConfig" in {
      ...
      verify(appConfig).preValidateTraderETDSUrl
    }
    ...
    "start and stop metrics" in {
      ...
      verify(metrics).timer(eqTo("etds.prevalidatetrader.connector.timer"))
      verify(metrics.timer(eqTo("etds.prevalidatetrader.connector.timer"))).time()
      verify(timerContext).stop()
    }
  }

  private def submitPreValidateTrader(): Future[Either[Result, ExciseTraderValidationETDSResponse]] =
