...
class MessageServiceSpec
  ...
  "temporary tests of private functions to build it up" should {
    "not save the substring LRN message" in {

    }
  }

  "There is a movement with an lrn, and we try to add a message with an lrn that is a substring of that lrn" should {
    "not add that message, because it doesn't belong on that movement - it should create a new movement instead" in {
      ...
      val messagesWithSubstringLrn = Seq(
        ...
        Message(utils.encode(new801WithSubstringLrn.head.toXml.toString()), "IE801", "GB00001", "testConsignee", Set.empty, now)
      )
