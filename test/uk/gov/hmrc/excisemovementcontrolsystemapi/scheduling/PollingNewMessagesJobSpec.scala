...
class PollingNewMessagesJobSpec
  ...
  private val newMessageResponse = ShowNewMessageResponse(
  ...
  override def beforeEach(): Unit = {
    ...
    when(movementService.getUniqueConsignorId).thenReturn(Future.successful(cachedMovements))
  ...
  "Job" should {
    ...
    "not process any message if no pending message exist" in {
      when(movementService.getUniqueConsignorId).thenReturn(Future.successful(Seq.empty))
    ...
    "return an error if there is an exception" in {
      when(movementService.getUniqueConsignorId).thenReturn(Future.failed(new RuntimeException("error")))
