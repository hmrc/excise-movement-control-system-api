...
import uk.gov.hmrc.excisemovementcontrolsystemapi.models.preValidateTrader.response.PreValidateTraderETDSEISResponse
...
class PreValidateTraderETDSHttpReader(
...
) extends HttpReads[Either[Result, PreValidateTraderETDSEISResponse]]
    with Logging
  ...
  ): Either[Result, PreValidateTraderETDSEISResponse] = {
  ...
  def extractIfSuccessful(response: HttpResponse): Either[HttpResponse, PreValidateTraderETDSEISResponse] =
    if (is2xx(response.status)) Right(extractResponse(response))
    else Left(response)

  private def extractResponse(httpResponse: HttpResponse): PreValidateTraderETDSEISResponse =
    jsonAs[PreValidateTraderETDSEISResponse](httpResponse.body)
  ...
  private def handleErrorResponse(
    ...
    val ourErrorResponse = EisErrorResponsePresentation(
      ...
      "Error occurred during ETDS PreValidateTrader request",
      correlationId
    )
