      ...
      .map { response: HttpResponse =>
        response.status match {
          case ACCEPTED =>
            ...
            submissionId
          case _ =>
            logger.warn(s"[NrsConnector] - Error when submitting to Non repudiation system (NRS) with status: ${response.status}, body: ${response.body}, correlationId: $correlationId")
