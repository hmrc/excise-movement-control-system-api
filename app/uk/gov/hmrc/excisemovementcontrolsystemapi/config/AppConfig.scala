...
class AppConfig @Inject() (config: Configuration, servicesConfig: ServicesConfig) {
  ...
  lazy val interval: FiniteDuration = config
    .getOptional[String]("scheduler.pollingNewMessageJob.interval")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
    .getOrElse(FiniteDuration(1, MINUTES))

  lazy val initialDelay: FiniteDuration = config
    .getOptional[String]("scheduler.pollingNewMessageJob.initialDelay")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
    .getOrElse(FiniteDuration(60, SECONDS))

  lazy val workItemInProgressTimeOut: FiniteDuration = config
    .getOptional[String]("scheduler.workItems.inProgressTimeOut")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
    .getOrElse(FiniteDuration(5, MINUTES))

  // When a work item fails, this is how many times to retry it before permanently failing
  lazy val maxFailureRetryAttempts: Int =
    config.getOptional[Int]("scheduler.workItems.failureRetryAttempts").getOrElse(3)
  ...
  lazy val failureRetryAfter: FiniteDuration = config
    .getOptional[String]("scheduler.workItems.failureRetryAfter")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
    .getOrElse(FiniteDuration(5, MINUTES))

  // When on the fast interval, how many times to fast poll before switching to slow poll
  lazy val fastIntervalRetryAttempts: Int =
    config.getOptional[Int]("scheduler.workItems.fastIntervalRetryAttempts").getOrElse(3)

  // The fast interval is used after submitting a message
  lazy val workItemFastInterval: FiniteDuration = config
    .getOptional[String]("scheduler.workItems.fastInterval")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
    .getOrElse(FiniteDuration(5, MINUTES))

  // The slow interval is used once fastInterval has been used for fastIntervalRetryAttempts times
  lazy val workItemSlowInterval: FiniteDuration = config
    .getOptional[String]("scheduler.workItems.slowInterval")
    .map(Duration.create(_).asInstanceOf[FiniteDuration])
  ...
  lazy val workItemTTL: Duration = config
    .getOptional[String]("mongodb.workItem.TTL")
    .fold(Duration.create(30, DAYS))(Duration.create(_).asInstanceOf[FiniteDuration])
